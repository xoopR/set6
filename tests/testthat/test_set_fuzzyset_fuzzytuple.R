...
test_that("construction", {
  ...
  expect_equal(FuzzyTuple$new(1, 0.1, 2, 0.8), FuzzyTuple$new(
    elements = c(1, 2),
...
test_that("inherited_methods", {
  ...
  expect_equal(FuzzyTuple$new(1, 0.4, 2, 0.9, class = "numeric")$max, 2)
  expect_equal(FuzzyTuple$new(1, 0.1, 2, 0.3, 3, 0.9)$elements, as.list(1:3))
...
test_that("support", {
  ...
  expect_equal(FuzzyTuple$new(1, 0.2, 3, 0.1, 2, 0)$support(), list(1, 3))
...
test_that("core", {
  ...
  expect_equal(FuzzyTuple$new(1, 1, 2, 0.3, 3, 1, 4.2, 0.1)$core(), list(1, 3))
})
test_that("inclusion", {
...
test_that("absolute complement", {
  expect_equal(setcomplement(FuzzyTuple$new(1, 0.1, 2, 0.8)), FuzzyTuple$new(1, 0.9, 2, 0.2))
})
...
test_that("isSubset", {
  ...
  expect_true(Set$new(1) < FuzzyTuple$new(elements = 1:3))
...
test_that("as.FuzzyTuple", {
  ...
  expect_equal(as.FuzzyTuple(list(elements = c(1, 2, 3), membership = c(0.1, 0.2, 0.3))), f)
  expect_equal(as.FuzzyTuple(data.frame(c(1, 2, 3), c(0.1, 0.2, 0.3))), f)
  expect_true(as.FuzzyTuple(Interval$new(1, 5, class = "integer")) == FuzzyTuple$new(elements = 1:5))
  expect_error(expect_equal(as.FuzzyTuple(Interval$new()), Interval$new()), "Interval cannot be")
  expect_error(expect_equal(as.FuzzyTuple(ConditionalSet$new(function(x) TRUE)), ConditionalSet$new(function(x) TRUE)), "ConditionalSet cannot be")
})
