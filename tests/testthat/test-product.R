...
test_that("Interval",{
  ...
  expect_equal(i$liesInSet(list(Tuple$new(1, 4), Tuple$new(1, 3), Tuple$new(3, 1))), c(TRUE, TRUE, FALSE))
...
test_that("ProductSet",{
  ...
  expect_true(i$liesInSet(c(Tuple$new(1,3,5), Tuple$new(2,4,5)), all = TRUE))
  expect_false(i$liesInSet(c(Tuple$new(0,3,5), Tuple$new(2,4,5)), all = TRUE))
  expect_false(i$liesInSet(Tuple$new(0,3)))
})
